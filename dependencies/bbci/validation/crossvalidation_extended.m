function [loss, lossSem, cfy_out, stats]= crossvalidation_extended(fv, varargin)
%CROSSVALIDATION - Perform cross-validation
% original BBCI crossvalitation function extended to provide F1, precision and recall 
% by Alexander von Lühmann, 2020
%
%Synopsis:
...
%  STATS   - struct that provides additional statistics: precision, recall, F1 score
% Examples:
...
for rr= 1:length(divTr),
  ...
  for ff= 1:nFolds,
    ...
    %% added in this extended version: Calc precision, recall, F1
    cout = sign(out);
    %true "negatives" (left)
    stats.TN(rr,ff) = numel(intersect(find(fvTe.y(1,:)), find(cout==-1)));
    %true "positives" (right)
    stats.TP(rr,ff) = numel(intersect(find(fvTe.y(2,:)), find(cout==1)));
    %false "negatives" (left)
    stats.FN(rr,ff) = numel(intersect(find(fvTe.y(2,:)), find(cout==-1)));
    %false "positives" (right)
    stats.FP(rr,ff) = numel(intersect(find(fvTe.y(1,:)), find(cout==1)));
